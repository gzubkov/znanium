...
class mod_znanium_mod_form extends moodleform_mod {
    function definition() {
        global $CFG, $DB;
        $mform = $this->_form;
        ...
        //-------------------------------------------------------
        $mform->addElement('header', 'content', get_string('contentheader', 'znanium'));
        $mform->addElement('text', 'bookid', get_string('bookid', 'znanium'), array('size'=>'10'));
        $mform->setType('bookid', PARAM_RAW_TRIMMED);
        ...
        if (count($options) == 1) {
            ...
            $mform->addElement('select', 'display', get_string('displayselect', 'znanium'), $options);
            $mform->setDefault('display', $config->display);
            $mform->addHelpButton('display', 'displayselect', 'znanium');
        }

        if (array_key_exists(RESOURCELIB_DISPLAY_POPUP, $options)) {
            $mform->addElement('text', 'popupwidth', get_string('popupwidth', 'znanium'), array('size'=>3));
            if (count($options) > 1) {
            ...
            $mform->addElement('text', 'popupheight', get_string('popupheight', 'znanium'), array('size'=>3));
            if (count($options) > 1) {
        ...
        if (array_key_exists(RESOURCELIB_DISPLAY_AUTO, $options) or
          ...
          array_key_exists(RESOURCELIB_DISPLAY_FRAME, $options)) {
            $mform->addElement('checkbox', 'printintro', get_string('printintro', 'znanium'));
        ...
        //-------------------------------------------------------
/*        $mform->addElement('header', 'parameterssection', get_string('parametersheader', 'znanium'));
        $mform->addElement('static', 'parametersinfo', '', get_string('parametersheader_help', 'znanium'));
        ...
        $options = znanium_get_variable_options($config);
        ...
        for ($i=0; $i < $parcount; $i++) {
            ...
            $mform->addGroup($group, $pargroup, get_string('parameterinfo', 'znanium'), ' ', false);
...
*/
    ...
    function validation($data, $files) {
        ...
        // Validating Entered znanium, we are looking for obvious problems only,
        ...
        // NOTE: do not try to explain the difference between znanium and URI, people would be only confused...
